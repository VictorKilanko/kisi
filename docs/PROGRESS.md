...
| Item | State |
|---|---|
| Current phase | **All three sites LIVE (2026-08-20).** pnpm+turbo **monorepo**: **www.kisi.africa** (`apps/africa`, entertainment, repositioned as the universe front door, no commerce), **farm.kisi.africa** (`apps/farm`, the business), **kids.kisi.africa** (`apps/kids`, **built out 2026-08-21** — home + `/stories` `/heroes` `/words` `/songs` `/printables`, hosted by Dede + Zizi), cast in `packages/canon`, tokens in `packages/brand`. **Mascot renamed Taco -> Dede (he/him).** See `docs/SPLIT_PLAN.md` (live checklist) + `docs/MONOREPO.md` (Vercel guide) + `kisikids/KIDS_PLAN.md` (kids runbook). |
| Kisi Kids | **Website built 2026-08-21.** `apps/kids`: read-along fables, factual hero profiles, Yoruba/Swahili word cards, songs (lyrics), print-friendly SVG activity sheets; content typed in `apps/kids/content/*.ts`; hosts Dede + **Zizi** (new canon hen, Dede's niece, cast now **28**). Accuracy audit CLEAN (Wangari Maathai facts, language words, tortoise fable all verified against sources). Video-first long-term; `kisikids/` folder holds the plan + video pipeline. |
...
| PWA | **Installable PWA (2026-07-31):** `app/manifest.ts`, `public/sw.js` (offline), Dede-face icons, `/offline`. See [[LESSONS]] |
| Monetisation funnel | **kisi.africa is entertainment-only now:** all buying + sponsoring funnels OUT to Kisi Farm via Explore buttons (home, economy, story arcs, chicken profiles, footer). `/shop` + `/support` 308-redirect to farm.kisi.africa. Selling lives on Kisi Farm; sponsorship still sandbox-locked |
| Latest content | **Season 2 Eps 8-9 built + staged 2026-08-21** (`the-second-chair` GREEN seeds Halima's heel turn; `the-league-kicks-off` CREAM, Chi-Chi's first league race). First batch under the brand split: sells point to **farm.kisi.africa**, slide wordmark "Kisi", mascot Dede. Showrunner SHIP, art director PASS. All historical story funnels migrated to farm.kisi.africa. Cast stays 27. | Earlier: Season 1 "The Fence Line" (7 eps) done + posted; Season 2 Eps 1 to 3 posted via cron (rebuild, sweetbeak, dawn all `posted` on `origin/main`). **Season 2 Ep 4 "Sweet Beak Strikes Again" built + staged 2026-08-13** (villain #2 escalation: Ládùn weaponizes envy over Coop Three, Halima defends Cindy's Law, grievance meeting flips to a Better Housing rally; `arcId: the-sweet-beak-returns`, GREEN, routes to Better Housing). Showrunner SHIP, art director PASS. **Pushed to `main` + Vercel redeployed 2026-08-14; `arc-sweetbeak2` queued for IG (next cron slot).** **Cadence fix same day (owner):** all four Season 2 arcs re-dated to reveal 08-17…08-24 (right after the Fence Line finale, the continuity floor) instead of 08-20…09-30, so the website catches up to the IG drip within ~10 days. Reveal-filtered arc count 9 today → 13 by 08-23. Named cast still 27 (no new character). |
| Social kit | **`social/` folder** — 12 intro/cast/leader cards + arc carousels through Season 2 Ep 4 (`arc-rebuild`, `arc-sweetbeak`, `arc-dawn` all posted; `arc-sweetbeak2` green, 5 slides, staged) + a WANTED poster; `render.ps1`, `generate-arcs.mjs`, `captions.md`, `README.md`. Logo = Dede headshot; name = Kisi Farm. **Cron switched to once-daily (06:00 UTC / 2 AM Eastern) 2026-08-14** now the backlog is clear and to stay under Instagram's app request limit; `arc-sweetbeak2` is the only staged post, so it posts at the next daily slot (08-15 06:00 UTC) once the throttle clears. **Two attempts (07:20 cron + 12:40 dispatch) both hit Instagram's "action is blocked" throttle; owner chose to stop manual retries and let the daily cron do it 08-15 06:00 UTC (~17h cool-down).** Manifest stayed `staged` both times (no duplicate risk) |
